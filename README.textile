...
I strongly believe that small targeted tools that do one thing well and refrain from doing anything else are the better way to go.

Each of the aforementioned libraries do way to much in my opinion. Amongst many other things they all implement tools for locating HTML elements from a Dom in some way. When we look at the details of the implementation they all do it differently though. So moving a test suite from Webrat to Capybara or Steam might be easy if only the simplest and most common helpers are used. But it can also be a huge pita because of all the tiny differences in those libraries.
...
* clarify that "Matchable values" refers to when a text selector is being used
* add notes about html class names being matched differently
* add notes about nested tags in content
